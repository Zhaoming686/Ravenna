...
# sbatch run_simple.sh dblp Ravenna test original "12 24 36 48 60" "0.2, 0,5, 0.8"
...
# Sixth argument: tau threshold; defaults to 0.3
TAU="${6:-0.3}"
...
echo -e "data_graph\tquery_graph\tt_ms\thard_sat\tcov_sat\tavg_rel_cov\tcall_count\tprune_count\tembedding_cnt\ttotal_embedding\ttotal_base_embedding\tvalid_base_embedding\tpi_f_embedding\tpi_d_embedding" > "$TSV_FILE"
      ...
      | tee -a "$RESULT_FILE" \
            | awk \
              ...
              BEGIN{
                ...
                total_embedding = -1;
                base_embedding = -1;
                valid_base_embedding = -1;
                pi_f_embedding = -1;
                pi_d_embedding = -1;
              }

              $1=="CHECK" && NF>=8 {
                checks[++ncheck] = dg_base OFS qg_base OFS $2 OFS $3 OFS $4 OFS $5 OFS $6 OFS $7 OFS $8;
              }
              ...
              $1=="total" && $2=="base" && $3=="embedding:" {
                base_embedding = $4;
              }

              $1=="valid" && $2=="base" && $3=="embedding:" {
                valid_base_embedding = $4;
              }
              ...
              END {
                for (i = 1; i <= ncheck; ++i) {
                  print checks[i], total_embedding, base_embedding, valid_base_embedding, pi_f_embedding, pi_d_embedding;
                }
              }' >> "$TSV_FILE"
  set -e
