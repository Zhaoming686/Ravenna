...
void BuildTable::buildTables(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count,
    ...
    for (auto u : build_table_order) {
        ...
        for (ui i = 0; i < u_nbrs_count; ++i) {
            ...
            // order
            for (ui row = 0; row < candidates_count[u_nbr]; ++row) {
                ui beg = edge_matrix[u_nbr][u]->offset_[row];
                ui end = edge_matrix[u_nbr][u]->offset_[row + 1];
                if (end > beg + 1) {
                    std::sort(edge_matrix[u_nbr][u]->edge_ + beg,
                              edge_matrix[u_nbr][u]->edge_ + end);
                }
            }
            ...
            // order
            for (ui row = 0; row < candidates_count[u]; ++row) {
                ui beg = edge_matrix[u][u_nbr]->offset_[row];
                ui end = edge_matrix[u][u_nbr]->offset_[row + 1];
                if (end > beg + 1) {
                    std::sort(edge_matrix[u][u_nbr]->edge_ + beg,
                              edge_matrix[u][u_nbr]->edge_ + end);
                }
            }
        }
