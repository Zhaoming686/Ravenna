...
void EvaluateQuery::SetTauValues(const std::vector<double> &tau_values, const Graph *data_graph)
{
    if (tau_values.empty())
    {
        throw std::invalid_argument("-tau must be provided as a single value or a comma-separated list.");
    }

    if (tau_values.size() > 1 && tau_values.size() != data_graph->getLabelsCount())
    {
        throw std::invalid_argument("-tau list size must equal the number of labels in the data graph.");
    }

    g_tau_values = tau_values;
}
                                ...
                                bool *visited_u, ui *order, TreeNode *tree)
{
    ...
    for (ui i = 0; i < query_vertices_num; i++)
    {
        if (visited_u[i] == true || extendable[i] != 0)
            continue;
        ui u_count = 0;
        for (ui j = 0; j < valid_cans_count[i]; j++)
        {
            if (visited_vertices[valid_cans[i][j]] == false)
                u_count++;
        }
    ...
    if (f_only_1 == false)
    {
        ...
        for (ui i = 0; i < query_vertices_num; i++)
        {
            if (visited_u[i] == true || nec[i] != 0 || extendable[i] != 0)
                continue;
...
// Add a function to calculate diversity
enumResult
EvaluateQuery::MMDIV(const Graph *data_graph, const Graph *query_graph, ui *&nodeId, Edges ***edge_matrix, ui **candidates,
                     ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, int TimeL, int FairT, const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    ...
    // Classify candidates for each label
    std::unordered_map<LabelID, std::vector<ui>> labelToCandidates;
    groupCandidatesByLabel(query_graph, qsiz, candidates, candidates_count, labelToCandidates);
    ...
    double last_sat = 0;           // the last recorded state, be used to fill in at EXIT
    double last_cov_sat = 0.0;     // min-based coverage satisfaction
    double last_avg_rel_cov = 0.0; // average relative coverage rate

    LabelID *labelsQuery = nullptr; // label set
    ui labelsQuerySize = 0;         // label set size
    BuildLabelsQuery(query_graph, qsiz, labelsQuery, labelsQuerySize);
    ui L = query_graph->getLabelsCount();
    ui *label_cover_cnt = new ui[L]; // the number of covered nodes for each label
    std::fill(label_cover_cnt, label_cover_cnt + L, 0);
    ui pruneCount = 0; // number of pruned embeddings due to upper bound
            ...
            { // if conflict on the last level then equivalence are match.
                ...
                { // enumerate the results if it has equivalent nodes.
                    if (VN[utemp].size() > 1)
                    {
                        ...
                        if (VNTemp.size() >= 1)
                        {
                            GreedyConstructEmbedding(data_graph, query_graph, candidates, order, VN, embedding, nodeId, label_flag, label_val, label_cover_cnt, embedding_cnt, cur_depth, true, v, &VNTemp, print_embedding);
...
EXIT:
    ...
    computeDiversity(query_graph, qsiz, candidates, candidates_count, nodeId, labelToCandidates, label_target);
    delete[] label_val;
    delete[] label_target;
    return s;
}

void EvaluateQuery::GreedyConstructEmbedding(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *order, std::vector<ui> *VN, ui *embedding, ui *nodeId, uint8_t *label_flag, int *label_val, ui *label_cover_cnt, size_t &embedding_cnt, int cur_depth, bool has_equiv, VertexID conflict_v, const std::vector<ui> *VNTempPtr, bool print_embedding)
{
    int qsiz = query_graph->getVerticesCount();

    std::vector<std::vector<ui>> temp_sol(qsiz);
    int UNPM = 0;
        ...
        while (ao >= 0)
        {
            ...
            if (VN[vqo].size() > 0)
            {
                for (int dd = 0; dd < VN[vqo].size(); dd++)
                {
                    if (VN[vqo][dd] != 10000000)
                    {
                        ui vv = candidates[vqo][VN[vqo][dd]];

                        if (nodeId[vv] == 0)
                            temp_sol[ao].push_back(vv);
                    }
                }
            }
    ...
    {
        temp_sol[cur_depth].push_back(conflict_v);
        ...
        while (ao >= 0)
        {
            ...
            if (embedding[vqo] != conflict_v)
                temp_sol[ao].push_back(embedding[vqo]);
            else
                temp_sol[ao].push_back(candidates[vqo][(*VNTempPtr)[0]]);
    ...
    while (OG)
    {
        ...
        if (UNPM > 0)
        {
            ...
            if (print_embedding)
            {
                std::cout << "GREEDY_EMB" << "\tUNPM=" << UNPM << "\t";

                for (int aa = 0; aa < qsiz; aa++)
                {
                    if (!temp_sol[aa].empty())
                    {
                        std::cout << order[aa] << ":" << temp_sol[aa][0];
...
void EvaluateQuery::computeDiversity(const Graph *query_graph, ui qsiz, ui **candidates, ui *candidates_count, ui *nodeId, std::unordered_map<LabelID, std::vector<ui>> labelToCandidates, const ui *label_target)
{
    // 1) label -> candidates
    std::unordered_map<LabelID, std::pair<ui, double>> labelStats;

    // 2) label -> candidates / covered embeddings
    std::unordered_map<LabelID, std::vector<ui>> labelToAllEmbeddings;

    // 3) group
    groupCoveredVerticesByLabel(query_graph, qsiz, candidates, candidates_count, nodeId, labelToAllEmbeddings);

    // 4) duplicate
    deduplicateLabelMap(labelToAllEmbeddings);

    // 5) sat
    recordAndPrintLabelCoverage(labelToCandidates, labelToAllEmbeddings, labelStats);

    // 6) average relative coverage rate
    recordAndPrintAverageRelativeCoverage(labelToCandidates, labelToAllEmbeddings, label_target);
}
// group based on candidates
void EvaluateQuery::groupCandidatesByLabel(const Graph *query_graph, ui qsiz, ui **candidates, ui *candidates_count, std::unordered_map<LabelID, std::vector<ui>> &labelToCandidates)
{
    for (ui i = 0; i < qsiz; ++i)
    {
        LabelID label = query_graph->getVertexLabel(i);
        auto &vec = labelToCandidates[label];
        for (ui j = 0; j < candidates_count[i]; ++j)
        {
            vec.push_back(candidates[i][j]);
        }
    }
}
// group based on covered vertices, nodeId
void EvaluateQuery::groupCoveredVerticesByLabel(const Graph *query_graph, ui qsiz, ui **candidates, ui *candidates_count, ui *nodeId, std::unordered_map<LabelID, std::vector<ui>> &labelToAllEmbeddings)
{
    for (ui i = 0; i < qsiz; ++i)
    {
        LabelID label = query_graph->getVertexLabel(i);
        auto &vec = labelToAllEmbeddings[label];
        for (ui j = 0; j < candidates_count[i]; ++j)
        {
            ui data_vertex = candidates[i][j];
            if (nodeId[data_vertex] == 1)
            {
...
// deduplicate entries in label map
void EvaluateQuery::deduplicateLabelMap(std::unordered_map<LabelID, std::vector<ui>> &labelMap)
{
...
// Record and output covered count and fraction for each label
void EvaluateQuery::recordAndPrintLabelCoverage(const std::unordered_map<LabelID, std::vector<ui>> &labelToCandidates, const std::unordered_map<LabelID, std::vector<ui>> &labelToAllEmbeddings, std::unordered_map<LabelID, std::pair<ui, double>> &labelStats)
{
    std::cout << "--- Label Coverage Statistics ---" << std::endl;
    ui cover_sum = 0;
    ui can_sum = 0;
    for (auto const &entry : labelToCandidates)
    {
        LabelID label = entry.first;
        ui total = entry.second.size();
        ui covered = labelToAllEmbeddings.count(label) ? labelToAllEmbeddings.at(label).size() : 0;
        double fraction = total > 0 ? (double)covered / total : 0;

        // Record both count and fraction
        labelStats[label] = {covered, fraction};
    ...
    std::cout << "The sum count of covered count : " << cover_sum
              << " | The sum count of candidates count " << can_sum << std::endl;
}
void EvaluateQuery::recordAndPrintAverageRelativeCoverage(const std::unordered_map<LabelID, std::vector<ui>> &labelToCandidates, const std::unordered_map<LabelID, std::vector<ui>> &labelToAllEmbeddings, const ui *label_target)
{
    double sum_rho = 0.0;
    ui label_count = 0;

    std::cout << "--- Average Relative Coverage Rate ---" << std::endl;
    ...
    for (auto const &entry : labelToCandidates)
    {
        ...
        {
            rho = static_cast<double>(covered) / static_cast<double>(target);
        }
        ...
        std::cout << "Label: " << label
                  ...
                  << " | Target: " << target
                  << " | Relative Coverage Rate: " << rho
                  << std::endl;
    }

    double avg_rho = label_count > 0 ? sum_rho / static_cast<double>(label_count) : 0.0;

    std::cout << "Average Relative Coverage Rate: " << avg_rho << std::endl;
}

// label_flag[label] = 1, show that this label has reached the threshold
void EvaluateQuery::InitQueryLabelFlag(const Graph *query_graph, uint8_t *&label_flag)
{
    ui L = query_graph->getLabelsCount(); // labels_count_ = max_label_id + 1

    label_flag = new uint8_t[L];
    std::fill(label_flag, label_flag + L, 0); // initial as 0
}

// initial label_val according to the candidates
void EvaluateQuery::InitLabelValByCandidates(const Graph *query_graph, const std::unordered_map<LabelID, std::vector<ui>> &labelToCandidates, int *&label_val, ui *&label_target)
{
    ui L = query_graph->getLabelsCount();
    label_val = new int[L];
    std::fill(label_val, label_val + L, -10000000);

    label_target = new ui[L];
    std::fill(label_target, label_target + L, 0);

    if (g_tau_values.empty())
    {
        throw std::invalid_argument("-tau must be provided as a single value or a comma-separated list.");
    }

    for (const auto &kv : labelToCandidates)
    {
        LabelID label = kv.first;
        if (g_tau_values.size() > 1 && label >= g_tau_values.size())
        {
            throw std::invalid_argument("query label id is outside the -tau list range.");
        }

        const auto &vec = kv.second; // already deduplicated
        size_t cnt = vec.size();
        double tau = g_tau_values.size() == 1 ? g_tau_values[0] : g_tau_values[label];

        ui tgt = (ui)std::ceil(tau * (double)cnt); // label threshold
        label_target[label] = tgt;

        int init = -(int)tgt;
        label_val[label] = init;
    }
}

void EvaluateQuery::UpdateLabelVal(const Graph *data_graph, ui Id, ui *nodeId, uint8_t *label_flag, int *label_val, ui *label_cover_cnt)
{
    if (nodeId[Id] == 1)
        return;

    nodeId[Id] = 1;
    LabelID l = data_graph->getVertexLabel(Id);
    label_cover_cnt[l] += 1;
    label_val[l] += 1;

    if (label_val[l] >= 0)
    {
        label_flag[l] = 1; // Reached the threshold
    }
}

void EvaluateQuery::UpdateLabelVal(const Graph *data_graph, ui Id, std::vector<bool> &KeyVertexSet, uint8_t *label_flag, int *label_val, ui *label_cover_cnt)
{
    if (KeyVertexSet[Id])
        return;

    KeyVertexSet[Id] = true;
    LabelID l = data_graph->getVertexLabel(Id);
    label_cover_cnt[l] += 1;
    label_val[l] += 1;
...
// generate valid candidates for current depth, if have neighbors with M, then intersect, if not use full set of CS
void EvaluateQuery::generateValidCandidateIndexPrune(bool *assigned, ui *candidates_count, ui depth, ui *idx_embedding, ui &idx_count, ui *&valid_candidate_index,
                                                     Edges ***edge_matrix, ui **bn, ui *bn_cnt, ui *order,
                                                     ui *&temp_buffer)
{
    ui countNumber = 0;
    ...
    VertexID previous_bn = -10000000;
    while (countNumber < bn_cnt[depth])
    {
        previous_bn = bn[depth][countNumber];
        if (assigned[previous_bn])
        {
            break; // found the first assigned neighbor
        }
        countNumber++;
    }
    if (countNumber >= bn_cnt[depth])
    { // No assigned neighbor found
        // Use full candidates
        for (ui i = 0; i < candidates_count[u]; ++i)
        {
            valid_candidate_index[i] = i;
        }
        idx_count = candidates_count[u];
    }
    else
    {
        ...
        if (bn_cnt[depth] > 0)
        {
            ...
            for (ui i = (countNumber + 1); i < bn_cnt[depth]; ++i)
            {
                VertexID current_bn = bn[depth][i];
                if (!assigned[current_bn])
                {
                    continue; // This neighbor is not matched yet
                }
                ui current_index_id = idx_embedding[current_bn];
                BSRGraph &cur_bsr_graph = *qfliter_bsr_graph_[current_bn][u];
                BSRSet &cur_bsr_set = cur_bsr_graph.bsrs[current_index_id];
        ...
        idx_count = valid_candidates_count;
        ...
        cout << "Ret, Level:" << bn_cnt[depth] << ", BSR:"
             << pretty_print_array(valid_candidate_index, valid_candidates_count)
             << "; GT: " << pretty_print_array(gt_valid_candidate_index, gt_valid_candidates_count) << "\n";

        for (auto i = 0; i < valid_candidates_count; i++)
        {
            assert(gt_valid_candidate_index[i] == valid_candidate_index[i]);
        }
        delete[] gt_valid_candidate_index;
#endif
#else
        ...
        memcpy(valid_candidate_index, previous_candidates, valid_candidates_count * sizeof(ui));
        ...
        for (ui i = (countNumber + 1); i < bn_cnt[depth]; ++i)
        {
            ...
            if (!assigned[current_bn])
            {
                continue; // the current neighbor is not assigned yet, next
            }
            ...
            ui current_candidates_count = current_edge.offset_[current_index_id + 1] - current_edge.offset_[current_index_id];
            ui *current_candidates = current_edge.edge_ + current_edge.offset_[current_index_id];
            ComputeSetIntersection::ComputeCandidates(current_candidates, current_candidates_count, valid_candidate_index, valid_candidates_count,
                                                      temp_buffer, temp_count);
            std::swap(temp_buffer, valid_candidate_index);
            valid_candidates_count = temp_count;
        }
...
// prune the candidates index based on symmetry breaking constraints
void EvaluateQuery::pruneCandidatesIndexBySymmetryBreakingCPrune(bool *assigned, ui depth, ui *embedding, ui *order,
                                                                 ui *candidates_count, ui **candidates,
                                                                 ui &idx_count, ui *&valid_candidates_idx,
                                                                 const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    ...
    if (it != ordered_constraints.end())
    {
        ...
        // If the lower and upper bounds conflict, prune to empty directly
        if (min_id > max_id)
        {
            idx_count = 0;
            return;
        ...
        {
            ui valid_idx = valid_candidates_idx[i];
            VertexID v = candidates[u][valid_idx];
            if (min_id <= v && v <= max_id)
            {
                valid_candidates_idx[valid_cnt++] = valid_idx;
            }
        }
        idx_count = valid_cnt;
    }
...
void EvaluateQuery::UpdateContinueFlags(const Graph *query_graph, bool &continueFlagBefore, bool &continueFlagAfter, ui cur_depth, ui max_depth, ui *order, const std::vector<ui> *VN, ui **valid_candidate_idx,
...
{
    // former layers
    // continueFlagBefore = false;
    continueFlagAfter = false; // the latter layers must be checks
    // former layers
    if (!continueFlagBefore)
    { // the former layers have no new vertices, we need to check
        int ao = cur_depth - 1;
        ...
        while (ao >= 0)
        {
            ...
            // First check whether the label for this row is still unsatisfied
            LabelID row_label = query_graph->getVertexLabel(vqo);
            if (label_flag[row_label] == 0)
            {
                // 1) check equivalent-class vertices in VN
                if (!VN[vqo].empty())
                {
                    for (ui dd = 0; dd < VN[vqo].size(); ++dd)
                    {
                        ui idx_v = VN[vqo][dd];
                        if (idx_v == 10000000)
                            continue;

                        VertexID cand_v = candidates[vqo][idx_v];
                        if (nodeId[cand_v] != 1) // not covered
                        {
                            continueFlagBefore = true;
                            break;
                        }
                    }
                }
                ...
                // // 2) check subset-class vertices in VSub
                // if (!VSub[vqo].empty())
                // {
                //     for (ui dd = 0; dd < VSub[vqo].size(); ++dd)
                //     {
                //         ui idx_v = VSub[vqo][dd];
                //         if (idx_v == 10000000) continue;
                //
                //         VertexID cand_v = candidates[vqo][idx_v];
                //         if (nodeId[cand_v] != 1) // not covered
                //         {
                ...
                // if (continueFlagBefore) break;
            }
            ao--;
        }
    }
    if (!continueFlagBefore)
    { // the former layers have no new vertices, then latter layers
        LabelID cur_row_label = query_graph->getVertexLabel(order[cur_depth]);
        if (label_flag[cur_row_label] == 0)
        {
            for (ui i = 0; i < idx_count[cur_depth]; i++)
            {
                ui valid_idxN = valid_candidate_idx[cur_depth][i];
                if (valid_idxN == 10000000)
                {
                    continue;
                }
                if (nodeId[candidates[order[cur_depth]][valid_idxN]] != 1)
                {
                    continueFlagAfter = true;
                    break;
                }
            }
        }
        ...
        // latter layers
        if (!continueFlagAfter)
        { // latter layers, util we find new vertices

            int ao = cur_depth + 1; // next depth
            while (ao < max_depth)
            {
                VertexID u = order[ao];
                LabelID row_label = query_graph->getVertexLabel(u);

                //  The label for this row is already satisfied
                if (label_flag[row_label] == 1)
                {
                    ao++;
                    continue;
                }

                ui valid_count_temp = 0;

                generateValidCandidateIndexPrune(assigned, candidates_count, ao, idx_embedding, valid_count_temp,
                                                 valid_candidate_idx_temp, edge_matrix, bn, bn_count, order, temp_buffer);

                pruneCandidatesIndexBySymmetryBreakingCPrune(assigned, ao, embedding, order, candidates_count, candidates,
                                                             valid_count_temp, valid_candidate_idx_temp, ordered_constraints);

                if (valid_count_temp == 0)
                {
                    continueFlagBefore = false;
                    continueFlagAfter = false;
                    break; // empty
                }

                for (ui i = 0; i < valid_count_temp; i++)
                {
                    ui valid_idx = valid_candidate_idx_temp[i];
                    VertexID v = candidates[u][valid_idx];
                    if (nodeId[v] != 1)
                    { // not covered
                        continueFlagAfter = true;
...
// labelsQuery's size is qsiz，labelsQuerySize <= qsiz
void EvaluateQuery::BuildLabelsQuery(const Graph *query_graph, ui qsiz, LabelID *&labelsQuery, ui &labelsQuerySize)
{
    labelsQuery = new LabelID[qsiz];
    labelsQuerySize = 0;

    for (ui i = 0; i < qsiz; ++i)
    {
        LabelID l = query_graph->getVertexLabel(i);

        // check the same
        bool exist = false;
        for (ui k = 0; k < labelsQuerySize; ++k)
        {
            if (labelsQuery[k] == l)
            {
                exist = true;
        ...
        if (!exist)
        {
            labelsQuery[labelsQuerySize++] = l;
        }
    }
}

void EvaluateQuery::DestroyLabelsQuery(LabelID *&labelsQuery)
{
    delete[] labelsQuery;
    labelsQuery = nullptr;
}
...
void EvaluateQuery::ComputeSatCovAndRelCov(double &last_sat, double &last_cov_sat, double &last_avg_rel_cov, const LabelID *labelsQuery, ui labelsQuerySize, const uint8_t *label_flag, const ui *label_cover_cnt, const ui *label_target)
{
    ...
    for (ui i = 0; i < labelsQuerySize; ++i)
    {
        ...
        {
            rho = static_cast<double>(label_cover_cnt[l]) / static_cast<double>(target);
        }

        if (target == 0 || label_flag[l] == 1 || label_cover_cnt[l] >= target)
        {
            hard_ok++;
        }

        cov_sat_sum += std::min(rho, 1.0);
        rel_cov_sum += rho;
    }
...
enumResult
EvaluateQuery::DSQLDIV(const Graph *data_graph, const Graph *query_graph, ui *&nodeId, Edges ***edge_matrix, ui **candidates,
                       ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, size_t &valid_vtx_cnt, int TimeL, int FairT,
                       const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
...
enumResult
EvaluateQuery::LFTJDLSDIV(const Graph *data_graph, const Graph *query_graph, ui *&nodeId, Edges ***edge_matrix, ui **candidates,
                          ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, size_t &valid_vtx_cnt, int TimeL, int FairT,
                          const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    ...
    while (K_size < max_depth)
    {
        ...
        while (hasNext == true)
        {
            ...
            while (count1 < max_depth)
            {
                ...
                {
                    if (count1 >= max_depth)
                        break; // Ensure we don't exceed max_depth
...
enumResult
EvaluateQuery::DIVSMSQDIV(const Graph *data_graph, const Graph *query_graph, ui *&nodeId, Edges ***edge_matrix, ui **candidates,
                          ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, int TimeL, int FairT, const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    bool print_embedding = false;
    // assumming that creating matrix can be done before we do not count the time.
    int qsiz = query_graph->getVerticesCount();
    int ksize = 1000;
    int SolPos[ksize + 1][qsiz];
            ...
            if (cur_depth != max_depth - 1) // change here
            {
                ...
                if (cur_depth != 0)
                {
                    ...
                    for (size_t read = 0; read < VN[u].size() && ia < idx_count[cur_depth];)
                    {
                        ...
                        if (VN[u][read] == valid)
                            VN[u][write++] = VN[u][read++], ++ia;
                        else if (VN[u][read] > valid)
                            ++ia;
                        else
                            ++read;
                    }
...
enumResult
EvaluateQuery::MatCoDIV(const Graph *data_graph, const Graph *query_graph, ui *&nodeId, Edges ***edge_matrix, ui **candidates,
                        ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, int TimeL, int FairT, const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    ...
    std::unordered_map<LabelID, std::vector<ui>> labelToCandidates;
    groupCandidatesByLabel(query_graph, query_graph->getVerticesCount(), candidates, candidates_count, labelToCandidates);
    deduplicateLabelMap(labelToCandidates);
    ...
    matco = new MatCo(query_graph, data_graph, output_limit_num, embedding_cnt, candidates, candidates_count, order, TimeL, labelToCandidates);
    ...
    // Write nodeId back using KeyVertexSet
    const auto &kv = matco->GetKeyVertexSet();
    ui n = data_graph->getVerticesCount();
    for (ui v = 0; v < n; ++v)
    {
        if (kv[v])
            nodeId[v] = 1;
...
static uint64_t g_op_count1 = 0;
static uint64_t g_op_count2 = 0;

size_t EvaluateQuery::outputEmbeddings(const std::vector<EmbeddingRecord> &embedding_records, ui qsiz, size_t output_limit_num, bool print_embedding)
{
    size_t output_cnt = 0;
    const ui INVALID_IDX = 10000000;

    auto printEmbedding = [&](const std::vector<ui> &base, const std::vector<Bucket> *vn_buckets, size_t column, VertexID replace_u, VertexID replace_v, bool use_single_replace) -> bool
    {
        if (output_cnt >= output_limit_num)
        {
            return false;
        }
        ...
        for (ui q = 0; q < qsiz; ++q)
        {
            ...
            if (vn_buckets != nullptr)
            {
                for (const auto &bucket : *vn_buckets)
                {
                    if (bucket.u == q && column < bucket.vertices.size() && bucket.vertices[column] != INVALID_IDX)
                    {
                        out_v = bucket.vertices[column];
                        break;
                    }
                }
            }

            if (use_single_replace && replace_u == q)
            {
                out_v = replace_v;
            }
    ...
    for (const auto &rec : embedding_records)
    {
        if (output_cnt >= output_limit_num)
        {
            break;
        ...
        // --------------------------------------------------------
        // 1. Output original embedding.
        // --------------------------------------------------------
        if (rec.output_original)
        {
            if (!printEmbedding(rec.embedding_by_q, nullptr, 0, 0, 0, false))
            {
                break;
            }
        }
        ...
        // --------------------------------------------------------
        // 2. Output VN diversified embeddings column by column.
        // --------------------------------------------------------
        ...
        for (size_t c = 0; c < vn_column_cnt; ++c)
        {
            if (!printEmbedding(rec.embedding_by_q, &rec.vn_buckets, c, 0, 0, false))
            {
                return output_cnt;
            }
        }
        ...
        // --------------------------------------------------------
        // 3. Output local-candidate diversified embeddings.
        //    Local buckets are also output column by column.
        //    Each column may replace multiple query vertices.
        // --------------------------------------------------------
        size_t local_column_cnt = 0;

        for (const auto &bucket : rec.local_buckets)
        {
            if (bucket.vertices.size() > local_column_cnt)
            {
                local_column_cnt = bucket.vertices.size();
            }
        }

        for (size_t c = 0; c < local_column_cnt; ++c)
        {
            if (!printEmbedding(rec.embedding_by_q, &rec.local_buckets, c, 0, 0, false))
            {
                return output_cnt;
            }
        }
...
void EvaluateQuery::markSubBySet(ui depth, VertexID u, ui **candidates, ui **valid_candidate_idx, uint8_t **valid_candidate_tag, ui *idx, ui *idx_count, ui *nodeId, const std::vector<ui> &prune_list)
{
    const ui INVALID_IDX = 10000000;
    const uint8_t TAG_SUB_2ND = 1;

    int kk = idx[depth];
    int dd = 0;
    int kk_end = idx_count[depth];
    int dd_end = (int)prune_list.size();

    while (kk < kk_end && dd < dd_end)
    {
        ui valid_idxn = valid_candidate_idx[depth][kk];
        if (valid_idxn == INVALID_IDX)
        {
            ++kk;
            continue;
        }

        ui prune_idxn = prune_list[dd];

        if (valid_idxn < prune_idxn)
        {
            ++kk;
        }
        else if (valid_idxn > prune_idxn)
        {
            ++dd;
        }
        else
        {
            ui data_v = candidates[u][valid_idxn];

            if (nodeId[data_v] == 1)
            {
                // prune directly
                valid_candidate_idx[depth][kk] = INVALID_IDX;
                g_op_count2++;
            }
            else if (valid_candidate_tag[depth][kk] == 0)
            {
                // put into the second pass
                valid_candidate_tag[depth][kk] = TAG_SUB_2ND;
                g_op_count2++;
            }
...
void EvaluateQuery::markSupBySet(ui depth, ui **valid_candidate_idx, uint8_t **valid_candidate_tag, ui *idx, ui *idx_count, const std::vector<ui> &prune_list)
{
    int kk = idx[depth];
    int dd = 0;
    int kk_end = idx_count[depth];
    int dd_end = (int)prune_list.size();
    const uint8_t TAG_SUP_2ND = 2;
    ...
    while (kk < kk_end && dd < dd_end)
    {
        ...
        {
            valid_candidate_tag[depth][kk] = TAG_SUP_2ND;
            g_op_count2++;
            ++kk;
            ++dd;
        }
...
void EvaluateQuery::pruneBySet(ui depth, VertexID u, ui **candidates, ui **valid_candidate_idx, ui *idx, ui *idx_count, ui *nodeId, const std::vector<ui> &prune_list, bool covered_only)
{
    int kk = idx[depth];
    int dd = 0;
    int kk_end = idx_count[depth];
    ...
    while (kk < kk_end && dd < dd_end)
    {
        ...
        if (valid_idxn == prune_idxn)
        {
            if (!covered_only || nodeId[candidates[u][valid_idxn]] == 1)
            {
                valid_candidate_idx[depth][kk] = 10000000;
...
void EvaluateQuery::calculateCellAndSet(const Graph *query_graph, Edges ***edge_matrix, ui *candidates_count, size_t **&candidatesHC, std::unordered_map<size_t, std::vector<ui>> *&idToValues, int count2, int i, int j)
{
    ui u_nbrs_count = 0;
    const VertexID *u_nbrs = query_graph->getVertexNeighbors(i, u_nbrs_count);
    ...
    // Only check VN: row(k) == row(j)
    auto rows_equal = [](
                          const ui *J, ui lenJ,
                          const ui *K, ui lenK) -> bool
    {
        if (lenJ != lenK)
            return false;
    ...
    for (ui k = static_cast<ui>(j + 1); k < candidates_count[i]; ++k)
    {
        bool global_equal = true;

        // Each equivalence class allows only one assignment
        bool can_assign_equal = (candidatesHC[i][k] == 0);

        for (ui d = 0; d < u_nbrs_count; ++d)
        {
            ui CVQ = u_nbrs[d];

            Edges *E = edge_matrix[i][CVQ];
            if (E == nullptr)
            {
                global_equal = false;
                break;
            ...
            ui j_beg = E->offset_[j];
            ui j_end = E->offset_[j + 1];
            ui k_beg = E->offset_[k];
            ui k_end = E->offset_[k + 1];

            const ui *J = E->edge_ + j_beg;
            const ui *K = E->edge_ + k_beg;
            ui lenJ = j_end - j_beg;
            ui lenK = k_end - k_beg;

            bool local_equal = rows_equal(J, lenJ, K, lenK);
        ...
        if (global_equal && can_assign_equal)
        {
            candidatesHC[i][k] = count2;
            idToValues[i][count2].push_back(k);
        }
...
void EvaluateQuery::calculateCellAndSubset(const Graph *query_graph, Edges ***edge_matrix, ui *candidates_count, size_t **&candidatesHC, std::unordered_map<size_t, std::vector<ui>> *&idToValues, std::unordered_map<size_t, std::vector<ui>> *&idToSubValues, int count2, int i, int j, std::unordered_map<size_t, std::vector<ui>> *&idToSupValues)
{
    ui u_nbrs_count = 0;
    const VertexID *u_nbrs = query_graph->getVertexNeighbors(i, u_nbrs_count);
                                 ...
                                 )
    {
        ...
        while (p < lenJ && q < lenK)
        {
            ...
            {
                // J has it but K does not => K cannot be a superset of J
                eq = false;
                ++p;
            }
            else // a > b
            {
                // K has it but J does not => K cannot be a subset of J
                eq = false;
                subset = false;
                ++q;
            }
    ...
    {
        bool is_equal = true;
        bool is_subset = true; // row(k) ⊆ row(j) on all neighbors
        ...
        for (ui d = 0; d < u_nbrs_count; ++d)
        {
            ...
            bool local_equal, local_subset, local_superset;
            compare_rows_one_pass(
                J, lenJ,
                K, lenK,
                local_equal, local_subset);

            is_equal &= local_equal;
            is_subset &= local_subset;
        ...
        // Each equivalence class allows only one assignment
        if (is_equal && can_assign_equal)
        {
            candidatesHC[i][k] = count2;
            idToValues[i][count2].push_back(k);
        }

        // Strict subsets / strict supersets; do not record equal vertices again
        if (is_subset && !is_equal)
        {
            idToSubValues[i][count2].push_back(k);
        }
    }
}

void EvaluateQuery::calculateCellFNAndSet(const Graph *query_graph, Edges ***edge_matrix, ui *candidates_count, size_t **&candidatesHC, std::unordered_map<size_t, std::vector<ui>> *&idToValues, int count2, int i, int j, int ROQ[])
{
    ui u_nbrs_count = 0;
    const VertexID *u_nbrs = query_graph->getVertexNeighbors(i, u_nbrs_count);
    const ui PO = ROQ[i];
    ...
    for (ui k = static_cast<ui>(j + 1); k < candidates_count[i]; ++k)
    {
        ...
        for (ui d = 0; d < u_nbrs_count; ++d)
        {
            ...
            if (ROQ[CVQ] < PO)
                continue; // Only check forward neighbors
...
void EvaluateQuery::calculateCellFNAndSubset(const Graph *query_graph, Edges ***edge_matrix, ui *candidates_count, size_t **&candidatesHC, std::unordered_map<size_t, std::vector<ui>> *&idToValues, std::unordered_map<size_t, std::vector<ui>> *&idToSubValues, int count2, int i, int j, int ROQ[], std::unordered_map<size_t, std::vector<ui>> *&idToSupValues)
{
    ui u_nbrs_count = 0;
    const VertexID *u_nbrs = query_graph->getVertexNeighbors(i, u_nbrs_count);
    const ui PO = ROQ[i];
    ...
    // Single pass checks simultaneously:
    // eq       : K == J
    // subset   : K ⊆ J
    // superset : K ⊇ J
    auto compare_rows_one_pass = [](
    ...
    {
        bool global_equal = true;
        bool global_subset = true; // row(k) ⊆ row(j) on all forward neighbors
        ...
        // Record only proper subsets / proper supersets, without duplicating equal vertices
        if (global_subset && !global_equal)
        {
            idToSubValues[i][count2].push_back(k);
        }
    }
                       ...
                       ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, int TimeL, int FairT, const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    ...
    computeAncestor(query_graph, bn, bn_count, order, ancestors);
    // label ancestor
    std::bitset<MAXIMUM_QUERY_GRAPH_SIZE> relevant_rows;
    relevant_rows.reset();
    ...
    // label ancestor
    relevant_rows.reset();
    for (ui qq = 0; qq < qsiz; ++qq)
    {
        LabelID lq = query_graph->getVertexLabel(qq);
        if (label_flag[lq] == 0)
        {
            relevant_rows |= ancestors[qq];
            relevant_rows.set(qq);
        }
    }
    ...
    // Prune flag, before and after
    bool continueFlagBefore = false;
    bool continueFlagAfter = false;

    // prune count because of the upper bound
    ui pruneCount = 0;

    bool *assigned = new bool[query_graph->getVerticesCount()]; // assigned flag for each query vertex
    std::fill(assigned, assigned + query_graph->getVerticesCount(), false);
    // calculate the maximum candidates_count
    ui max_candidates = 0;
    for (ui i = 0; i < max_depth; ++i)
    {
        if (candidates_count[order[i]] > max_candidates)
        {
            max_candidates = candidates_count[order[i]];
        }
    }
    ui *temp_buffer_prune = new ui[max_candidates];
    ...
    double last_sat = 0;           // last satisfaction ratio, be used to record and in EXIT
    double last_cov_sat = 0.0;     // min-based coverage satisfaction
    double last_avg_rel_cov = 0.0; // average relative coverage rate
    ...
    BuildLabelsQuery(query_graph, qsiz, labelsQuery, labelsQuerySize);
    ui L = query_graph->getLabelsCount();
    ui *label_cover_cnt = new ui[L]; // number of covered vertices for each label
    std::fill(label_cover_cnt, label_cover_cnt + L, 0);

    // for gobale exploration
    ui last_root_idx = std::numeric_limits<ui>::max();
    uint32_t epoch = 1;
    std::vector<uint32_t> ranked_epoch(max_depth, 0);

    // true inclusion set
    std::unordered_map<size_t, std::vector<ui>> *idToSubValues;
    idToSubValues = new std::unordered_map<size_t, std::vector<ui>>[qsiz];

    std::unordered_map<size_t, std::vector<ui>> *idToSupValues;
    idToSupValues = new std::unordered_map<size_t, std::vector<ui>>[qsiz];

    // [query vertex][label] = 10000000
    const int INIT_VAL = 10000000;
    int **ul_val = new int *[qsiz];
    for (ui u = 0; u < qsiz; ++u)
    {
        ul_val[u] = new int[L];
        std::fill(ul_val[u], ul_val[u] + L, INIT_VAL);
    }
    int *future_min_by_qv = new int[qsiz];
    std::fill(future_min_by_qv, future_min_by_qv + qsiz, INIT_VAL);
    std::vector<ui> vn_new_cnt(query_graph->getVerticesCount(), 10000000);

    bool *empty_subtree_rep = new bool[max_depth];
    std::fill(empty_subtree_rep, empty_subtree_rep + max_depth, false);

    bool *haveDiverisity = new bool[max_depth];
    std::fill(haveDiverisity, haveDiverisity + max_depth, false);

    // NEW: current working subset list for each query vertex
    vector<ui> VSub[query_graph->getVerticesCount()];
    vector<ui> VSubTemp;
    ...
    while (true) // main loop
    {
        ...
        continueFlagAfter = false;  //
        // current position smaller than size
        while (idx[cur_depth] < idx_count[cur_depth]) // every depth, two-pass
        {
            ...
            if (cur_depth == 0 && idx[cur_depth] != 0 && FairT > 0) // global exploration
            {
                if (FairT == 1)
                    rankSimple(order[0], nodeId, candidates, valid_candidate_idx, idx, valid_idx, idx_count);
                else if (FairT == 2)
            ...
            // Check whether u has any forward neighbor in the matching order.
            bool has_forward_neighbor = false;
            ui u_nbrs_count = 0;
            const VertexID *u_nbrs = query_graph->getVertexNeighbors(u, u_nbrs_count);
            ...
            for (ui d = 0; d < u_nbrs_count; ++d)
            {
                ...
                if (RQ[w] > cur_depth)
                {
                    has_forward_neighbor = true;
                    break;
                }
            }

            size_t candidate = candidatesHC[u][valid_idx];
            if (has_forward_neighbor && candidate == 0 && cur_depth == 0)
            {
                calculateCellAndSubset(query_graph, edge_matrix, candidates_count, candidatesHC,
                                       idToValues, idToSubValues, count2, u, valid_idx, idToSupValues);
                count2++;
            }
            else if (has_forward_neighbor && candidate == 0 && cur_depth != max_depth - 1)
            {
                calculateCellFNAndSubset(query_graph, edge_matrix, candidates_count, candidatesHC,
                                         idToValues, idToSubValues, count2, u, valid_idx, RQ, idToSupValues);
                count2++;
            }
            // candidate = candidatesHC[u][valid_idx];
            VN[u].clear();
            ...
            if (!has_forward_neighbor)
            {
                // No forward neighbor: all remaining local candidates are equivalent.
                // There is no strict subset relation, so VSub[u] stays empty.
                for (int ia = idx[cur_depth]; ia < idx_count[cur_depth]; ++ia)
                {
                    ui valid_idxN = valid_candidate_idx[cur_depth][ia];
                    ...
                    VN[u].push_back(valid_idxN);
                }
            }
            else
            {
                size_t cell_id = candidatesHC[u][valid_idx];
                VN[u] = idToValues[u][cell_id];
                VSub[u] = idToSubValues[u][cell_id];
                ...
                if (cur_depth != 0)
                {
                    ...
                    while (ia < idx_count[cur_depth] &&
                           (io_eq < (int)VN[u].size() || io_sub < (int)VSub[u].size()))
                    {
                        ui valid_idxN = valid_candidate_idx[cur_depth][ia];
                        ...
                        while (io_eq < (int)VN[u].size() && VN[u][io_eq] < valid_idxN)
                            io_eq++;

                        while (io_sub < (int)VSub[u].size() && VSub[u][io_sub] < valid_idxN)
                            io_sub++;

                        if (io_eq < (int)VN[u].size() && VN[u][io_eq] == valid_idxN)
                        {
                            VNTemp.push_back(valid_idxN);
                            io_eq++;
                        }

                        if (io_sub < (int)VSub[u].size() && VSub[u][io_sub] == valid_idxN)
                        {
                            VSubTemp.push_back(valid_idxN);
                            io_sub++;
                        }
            ...
            if (visited_vertices[v])
            {
                ...
                // Try to remap current query vertex u to an unvisited equivalent candidate.
                for (ui vn_idx : VN[u])
                {
                    VertexID vn_v = candidates[u][vn_idx];

                    if (!visited_vertices[vn_v])
                    {
                        valid_idx = vn_idx;
                        v = vn_v;
                        repaired = true;
                ...
                // If u cannot be repaired, try to move the previous owner utemp away.
                if (!repaired)
                {
                    for (ui vn_idx : VN[utemp])
                    {
                        VertexID vn_v = candidates[utemp][vn_idx];

                        if (!visited_vertices[vn_v])
                        {
                            embedding[utemp] = vn_v;
                            idx_embedding[utemp] = vn_idx;

                            // Update occupancy and reverse mapping.
                            visited_vertices[v] = false;
                            embd_vertices[v] = false;
                            reverse_embedding.erase(v);

                            visited_vertices[vn_v] = true;
                            embd_vertices[vn_v] = true;
                            reverse_embedding[vn_v] = utemp;
                ...
                // If neither side can be repaired, perform the normal intersection update
                // and mark this candidate as failed.
                if (!repaired)
                {
                    VNTemp.clear();
                    VSubTemp.clear();
                    ...
                    while (pu < (int)VN[u].size() &&
                           (pe < (int)VN[utemp].size() || ps < (int)VSub[utemp].size()))
                    {
                        ui x = candidates[u][VN[u][pu]];

                        while (pe < (int)VN[utemp].size() &&
                               candidates[utemp][VN[utemp][pe]] < x)
                        {
                            ++pe;
                        }

                        while (ps < (int)VSub[utemp].size() &&
                               candidates[utemp][VSub[utemp][ps]] < x)
                        {
                            ++ps;
                        }

                        if (pe < (int)VN[utemp].size() &&
                            candidates[utemp][VN[utemp][pe]] == x)
                        {
                            VNTemp.push_back(VN[utemp][pe]);
                            ++pe;
                        }

                        if (ps < (int)VSub[utemp].size() &&
                            candidates[utemp][VSub[utemp][ps]] == x)
                        {
                            VSubTemp.push_back(VSub[utemp][ps]);
                            ++ps;
                        }
                    ...
                    VN[utemp].swap(VNTemp);
                    VSub[utemp].swap(VSubTemp);
                    ...
                    vec_failing_set[cur_depth] = ancestors[u];
                    vec_failing_set[cur_depth] |= ancestors[utemp];
                    vec_failing_set[cur_depth - 1] |= vec_failing_set[cur_depth];

                    continue;
                }
            } // finish conflict check

            assigned[u] = true; // mark u as assigned
            ...
            if (cur_depth == max_depth - 1) // leaf node, construct buckets and mark
            {
                raw_embedding_cnt += 1;
            ...
            { // non-leaf node, continue to the next depth
                call_count += 1;
                cur_depth += 1;     // next depth
                idx[cur_depth] = 0; // set the element to 0
                // generateValidCandidateIndex(cur_depth, idx_embedding, idx_count, valid_candidate_idx, edge_matrix, bn,
                //                             bn_count, order, temp_buffer, candidates, candidates_count, query_graph);
                generateValidCandidateIndex(cur_depth, idx_embedding, idx_count, valid_candidate_idx, edge_matrix, bn,
                                            bn_count, order, temp_buffer); // Generate local candidates

                pruneCandidatesIndexBySymmetryBreakingC(cur_depth, embedding, order, candidates_count, candidates,
                                                        idx_count, valid_candidate_idx, ordered_constraints); // Prune local candidates with symmetry-breaking constraints to avoid enumerating symmetric solutions, such as duplicate enumeration from swapping two nodes

                // get the candidates for next depth
                if (idx_count[cur_depth] == 0) // next ther is no new candidates
                {
                    vec_failing_set[cur_depth - 1] = ancestors[order[cur_depth]];
                    empty_subtree_rep[cur_depth - 1] = true; // Mark the current representative subtree at the previous layer as empty
                    haveDiverisity[cur_depth - 1] = false;
                    // g_op_count1++;
                }
                else
                {
                    empty_subtree_rep[cur_depth - 1] = false;
                    haveDiverisity[cur_depth - 1] = false;
                    ui *tmp_idx = valid_candidate_idx_temp; // local copy
                    ui *tmp_buf = temp_buffer_prune;        // local copy
                    UpdateContinueFlags(query_graph, continueFlagBefore, continueFlagAfter, cur_depth, max_depth, order, VN,
                                        valid_candidate_idx, idx_count, candidates, candidates_count, nodeId, label_flag, data_graph,
                                        assigned, idx_embedding, embedding, edge_matrix, bn, bn_count, tmp_buf, tmp_idx, ordered_constraints);

                    if (!continueFlagBefore && !continueFlagAfter)
                    { // former and latter layers have no new vertices, we can directly prune
                        pruneCount++;
                        idx_count[cur_depth] = 0;
                        vec_failing_set[cur_depth - 1].set();
                        haveDiverisity[cur_depth - 1] = true;
                    }
                    else
                    {
                        vec_failing_set[cur_depth - 1].reset();
        ...
        else
        {
            ...
            {
                // if (!VN[vq].empty()) {
                //     pruneBySet(cur_depth, vq, candidates, valid_candidate_idx, idx, idx_count, nodeId, VN[vq], false);
                // }

                // 1) Original equivalence set pruning
                if (VN[vq].size() > 1) // just for the current depth, jump all the other vertices in the VN, prune
                {
                    for (int dd = 0; dd < VN[vq].size(); dd++)
                    {
                        for (int kk = idx[cur_depth]; kk < idx_count[cur_depth]; kk++)
                ...
                VN[vq].clear();
                // 2) VSub：
                // empty subtree or the current vertex label's label_flag==1 -> prune all
                // Otherwise prune only nodeId==1
                LabelID lq = query_graph->getVertexLabel(vq);
                if (empty_subtree_rep[cur_depth] || label_flag[lq] == 1)
                {
                    pruneBySet(cur_depth, vq, candidates, valid_candidate_idx, idx, idx_count, nodeId, VSub[vq], false);
                }
                else
                {
                    pruneBySet(cur_depth, vq, candidates, valid_candidate_idx, idx, idx_count, nodeId, VSub[vq], true);
                }
            }
            ...
            VertexID u = order[cur_depth];
            reverse_embedding.erase(embedding[u]);
            if (cur_depth != 0) // not root vertex, update failing set
            {
                if (!vec_failing_set[cur_depth].test(u))
                {
                    vec_failing_set[cur_depth - 1] = vec_failing_set[cur_depth];
...
EXIT:
    ...
    delete[] temp_buffer_prune;
    delete[] valid_candidate_idx_temp;
    delete[] idToSubValues;

    computeDiversity(query_graph, qsiz, candidates, candidates_count, nodeId, labelToCandidates, label_target); // compute diversity coverage

    delete[] label_val;
    delete[] label_target;
    std::cout << "The prune count: " << pruneCount << std::endl;

    std::cout << "total embedding: " << embedding_cnt << std::endl;
    std::cout << "total base embedding: " << embedding_count0 << std::endl;
    std::cout << "valid base embedding: " << embedding_count1 << std::endl;
    std::cout << "pi_f_embedding: " << embedding_count2 << std::endl;
    std::cout << "pi_d_embedding: " << embedding_count3 << std::endl;
                              ...
                              ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, int TimeL, int FairT, const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    ...
    while (true) // main loop
    {
        ...
        continueFlagAfter = false;  //
        // current position smaller than size
        while (idx[cur_depth] < idx_count[cur_depth]) // every depth
        {
            Match_BA[cur_depth] = false;
            auto end = std::chrono::high_resolution_clock::now();
            ens = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
            ...
            if (has_forward_neighbor && candidate == 0 && cur_depth == 0) // root node, the equivalence cell
            {
                calculateCellAndSet(query_graph, edge_matrix, candidates_count, candidatesHC, idToValues, count2, u, valid_idx);
                count2++;
            }
            // else if (candidate == 0 &&cur_depth != max_depth - 1)
            else if (has_forward_neighbor && candidate == 0 && cur_depth != max_depth - 1) // the equivalence for non-leaf node
            {
                calculateCellFNAndSet(query_graph, edge_matrix, candidates_count, candidatesHC, idToValues, count2, u, valid_idx, RQ);
                count2++;
            }
            // candidate = candidatesHC[u][valid_idx];
            VN[u].clear();
            ...
            else
            {
                ...
                if (cur_depth != 0)
                {
                    ...
                    while (ia < idx_count[cur_depth] && io < (int)VN[u].size())
                    {
                        ui valid_idxN = valid_candidate_idx[cur_depth][ia];
            ...
            if (visited_vertices[v])
            {
                ...
                if (!repaired)
                {
                    ...
                    while (pu < (int)VN[u].size() && pe < (int)VN[utemp].size())
                    {
                        ui x = candidates[u][VN[u][pu]];
                            ...
                            ui *candidates_count, ui *order, size_t output_limit_num, size_t &call_count, int TimeL, int FairT, const std::unordered_map<VertexID, std::pair<std::set<VertexID>, std::set<VertexID>>> &ordered_constraints)
{
    ...
    while (true) // main loop
    {
        ...
        else
        {
            ...
            {
                // Only prune the candidate idx that has already been matched successfully.
                ui matched_idx = idx_embedding[vq];
...
EXIT:
    ...
    std::cout << "Prune operation count1: " << g_op_count1 << std::endl;
    std::cout << "Prune operation count2: " << g_op_count2 << std::endl;
...
//     auto isUsefulNewVertex = [&](VertexID v) -> bool {
//         if (nodeId[v] != 0) {
//             return false;
//         }

//         LabelID l = data_graph->getVertexLabel(v);
//         return label_flag[l] == 0;
//     };

//     auto markIfUseful = [&](VertexID v) -> bool {
//         if (!isUsefulNewVertex(v)) {
//             return false;
//         }
...
//         ui beg = E->offset_[cand_idx_u];
//         ui end = E->offset_[cand_idx_u + 1];

//         const ui *row_begin = E->edge_ + beg;
//         const ui *row_end = E->edge_ + end;

//         return std::binary_search(row_begin, row_end, cand_idx_w);
//     };

//     auto satisfyForwardNeighbors = [&](VertexID u, int dep, ui cand_idx) -> bool {
//         ui nbr_cnt = 0;
//         const VertexID *nbrs = query_graph->getVertexNeighbors(u, nbr_cnt);

//         for (ui k = 0; k < nbr_cnt; ++k) {
//             VertexID w = nbrs[k];

//             if (RQ[w] > dep && RQ[w] <= max_dep) {
//                 ui cand_idx_w = base_idx_embedding[w];

//                 if (cand_idx_w == INVALID_IDX) {
//                     return false;
//                 }

//                 if (!hasEdgeToForwardNeighbor(u, w, cand_idx, cand_idx_w)) {
//                     return false;
//                 }
//             }
//         }
...
//     // ------------------------------------------------------------
//     // 1. Original embedding: only record it if it has useful gain.
//     // ------------------------------------------------------------
//     for (int dep = 0; dep <= max_dep; ++dep) {
//         VertexID u = order[dep];
//         VertexID v = base_embedding[u];
...
//         for (ui i = 0; i < VN[u].size(); ++i) {
//             ui cand_idx = VN[u][i];

//             if (cand_idx == INVALID_IDX) {
//                 continue;
//             }

//             VertexID v = candidates[u][cand_idx];
...
//     // ------------------------------------------------------------
//     // 3. VSub bucket: enumerate only true-subset candidates.
//     //    Each point here will later be output by single replacement.
//     // ------------------------------------------------------------
//     if (VSub != nullptr)
//     {
...
//                 if (!satisfyForwardNeighbors(u, dep, cand_idx))
//                 {
//                     continue;
//                 }

//                 getBucket(rec.local_buckets, u).push_back(v);
...
//     for (const auto &bucket : rec.local_buckets) {
//         output_cnt += bucket.vertices.size();
//         embedding_count3 += bucket.vertices.size();
//     }
        ...
        for (int dep = max_dep; dep >= 0; --dep)
        {
            ...
            for (ui i = 0; i < VSub[u].size(); ++i)
            {
                ...
                for (ui k = 0; k < nbr_cnt; ++k)
                {
                    ...
                    // Only repair forward query neighbors within the current embedding depth.
                    if (!(RQ[uf] > dep && RQ[uf] <= max_dep))
                    {
                        continue;
                    }

                    if (repair_idx_buckets[uf].empty())
                    {
                        feasible = false;
                        break;
                    }
                    ...
                    for (ui cand_idx_f : repair_idx_buckets[uf])
                    {
                        if (cand_idx_f == INVALID_IDX)
                        {
                            continue;
                        }

                        if (!hasEdgeToForwardNeighbor(u, uf, cand_idx, cand_idx_f))
                        {
                            continue;
                        }

                        VertexID vf = candidates[uf][cand_idx_f];

                        replacements.emplace_back(uf, vf);
                        found = true;
                        break;
                    }
