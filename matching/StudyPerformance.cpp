...
std::vector<double> parseTauValues(const std::string &tau_arg)
{
    std::vector<double> values;
    std::stringstream ss(tau_arg);
    std::string item;
...
int main(int argc, char **argv)
{
    ...
    std::vector<double> tau_values = parseTauValues(command.getTau());
    // int QNLR=stoi(QNL);
    // int QNR=stoi(QN);
    int QNLR = 1;
    int QNR = 1;
    ...
    EvaluateQuery::SetTauValues(tau_values, data_graph);
    ...
    for (QNR; QNR <= QNLR; QNR++)
    {
        ...
        {
            s = EvaluateQuery::DSQLDIV(data_graph, query_graph, nodeID, edge_matrix, candidates, candidates_count,
                                       matching_order, output_limit, call_count, valid_vtx_count, TimeL1, FairT,
                                       ordered_constraints);
        }
        else if (input_engine_type == "DSQLPDIV")
        {

            s = EvaluateQuery::LFTJDLSDIV(data_graph, query_graph, nodeID, edge_matrix, candidates, candidates_count,
                                          matching_order, output_limit, call_count, valid_vtx_count, TimeL1, FairT,
                                          ordered_constraints);
        }
        else if (input_engine_type == "MMDIV")
        {
            s = EvaluateQuery::MMDIV(data_graph, query_graph, nodeID, edge_matrix, candidates, candidates_count, matching_order,
                                     output_limit, call_count, TimeL1, FairT, ordered_constraints);
        }
        else if (input_engine_type == "MMIDIV")
        {
            s = EvaluateQuery::DIVSMSQDIV(data_graph, query_graph, nodeID, edge_matrix, candidates, candidates_count, matching_order,
                                          output_limit, call_count, TimeL1, FairT, ordered_constraints);
        }
        else if (input_engine_type == "MATCODIV")
        {

            s = EvaluateQuery::MatCoDIV(data_graph, query_graph, nodeID, edge_matrix, candidates, candidates_count, matching_order,
                                        output_limit, call_count, TimeL1, FairT, ordered_constraints);
        }
        else if (input_engine_type == "RavennaCheck")
        {
            s = EvaluateQuery::RavennaCheck(data_graph, query_graph, nodeID, edge_matrix, candidates, candidates_count, matching_order,
                                            output_limit, call_count, TimeL1, FairT, ordered_constraints);
        }
        else if (input_engine_type == "RavennaForward")
        {
            s = EvaluateQuery::RavennaForward(data_graph, query_graph, nodeID, edge_matrix, candidates, candidates_count, matching_order,
                                              output_limit, call_count, TimeL1, FairT, ordered_constraints);
        }
        else if (input_engine_type == "Ravenna")
        {
            s = EvaluateQuery::Ravenna(data_graph, query_graph, nodeID, edge_matrix, candidates, candidates_count, matching_order,
                                       output_limit, call_count, TimeL1, FairT, ordered_constraints);
        }
        else
        {
