...
bool GenerateQueryD(ui QS, const Graph *data_graph, ui MinDen, int dd, const string& out_dir)
{
    ...
    if (((ED * 2) / QS) > 0)
    {
        ...
        std::ofstream outputFile(out_dir + "/query_G_"
                                 + to_string(QS) + "_" + to_string(dd) + ".graph");
...
bool GenerateQueryS(ui QS, const Graph *data_graph, ui MinDen, int dd,
                    bool stop, const string& out_dir)
{
    unordered_map<ui, ui> SID;
    vector<ui> ID;
    ui ED = 0;
    ...
    if (((ED * 2) / QS) <= MinDen)
    {
        std::ofstream outputFile(out_dir + "/query_sparse_"
                                 + to_string(QS) + "_" + to_string(dd) + ".graph");
